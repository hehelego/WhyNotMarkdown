...
# toggle proxy
config.bind(
    f'{leader}tp',                                    'config-cycle -tp content.proxy http://127.0.0.1:8889 socks5://127.0.0.1:1089 none')
...
# download page
config.bind(f'{leader}d',                             'download', mode='normal')
config.bind(f'{leader}D',                             'hint links download', mode='normal')
...
config.bind('<Alt-p>',                                f'spawn --userscript qute-keepassxc --key {qutekey}', mode='insert')
...
config.bind(f'{leader}z',                             'config-cycle -tp statusbar.show never always ;; config-cycle -tp tabs.show never always ;; clear-messages ;; download-clear')

##################################### PART C: userscripts #####################################

# run the developing userscript
config.bind(f'{leader}T',                             'spawn --userscript test.py', mode='normal')

# spinach's dinctionary lookup plugin: search for the selected text in YouDao dinctionary
config.bind(f'{leader}{leader}',                      'spawn --userscript spinach-dictlookup.py', mode='normal')
# spinach's bookmarks selector: open bookmark, with full feature
config.bind(f'{leader}b',                             'spawn --userscript spinach-bookmarks.py --full', mode='normal')
config.bind(f'{leader}B',                             'spawn --userscript spinach-bookmarks.py --full', mode='normal')
...
config.bind(f'{leader}s',                             'spawn --userscript spinach-themes.py', mode='normal')
