...
对于第二类斯特林数,有一个非常有用的性质,用于将$\sum a_ix^i$转化为$\sum b_ix^{\underline i}$间转化.$m^n=\sum_{i=1}^{n}\begin{Bmatrix} n\\ i \end{Bmatrix}*m^{\underline{i}}$.对于$x^{\underline i}$到$x^i$的转化,则需要使用第一类斯特林数.可以看这里[onino_cyc blog:集合划分计数](<https://www.cnblogs.com/onioncyc/p/8722262.html>).
