...
$$
\begin{aligned}
(p>1)&\lim_{n\to \infty}\frac{\sum_{i=1}^n i^p}{n^p}
=\lim_{n\to \infty}\sum_{i=1}^n {(\frac{1}{n})}^p\\
&\lim_{n\to \infty}\sum_{i=0}^{n-1}\frac{1}{\sqrt{n^2 - i^2}}
=\lim_{n\to \infty}\frac{1}{n}\sum_{i=0}^{n-1}\frac{1}{\sqrt{1 - {(\frac{1}{n})}^2}}
\end{aligned}
$$

对于第一个式子,我们考虑$\int_{0}^{1} x^p\mathrm dx$,我们已知它是存在的,  
$\forall \epsilon>0\exist \delta>0,\text{ s.t. }\forall T,||T||<\delta\quad |S(T)-I|<\epsilon$  
而$\forall \delta,\exist N\text{ s.t. }\forall n>N\quad T_n=\{\frac{1}{n},\frac{2}{n}\ldots \frac{n}{n}\},||T_n||=\frac{1}{n}<\delta\implies |\sum(\frac{1}{n})^p-I|<\epsilon$  
于是这个和跟着积分一起收敛.  


下面一个稍微复杂,考虑$\int_0^1 \frac{\mathrm dx}{\sqrt{1-x^2}}$,这个东西是$\frac{\pi}{2}$,类似上面的论证,可以说明  
$\text{let }\epsilon=1,\exist N,\text{ s.t. }\forall n>N\quad |\sum(1-(\frac{1}{n}))^\frac{-1}{2}-\frac{\pi}{2}|< 1$,使用夹逼准则,和式收敛到$0$.  
