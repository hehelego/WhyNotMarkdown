...
> Def: Cover of a set.  
> Given a set $A$, a family $\{A_1,A_2\ldots A_k\}$ s.t. $A_i\subseteq A,\cup A_i=A$ is called a cover of $A$.(Unlike a **partition**, here, we do not require no-overlaping)  
> There is at least a set $A_k$ s.t. $|A_k|\geq \left\lfloor\frac{|A|}{k}\right\rfloor$
...
## LRR(linear recurrence relation), the generatingfunctionological way

We can solve LRRs by linear-ODE,OGF,EGF
...
\sum_{i=1}^k c_{i} x^{k-i} F(x)
&=\sum_{n=0}^\infty x^n\sum_{i=1}^k
\end{aligned}
$$
...
\sum_{i=1}^k c_i\frac{\mathrm{d}^i G(x)}{\mathrm{d} x^i}
\end{aligned}
$$
